...
(i.e. impossible). 
Our family of evaluators contains 
...
\citep{WalidICFP02} of error checking.  See \citep{Gluck-jones-optimality}
for deeper technical reasons which inevitably lead to these performance
penalties.
...
a technique due to Asai \ccshan{cite?}\jacques{yes}: build a dynamic term
...
function, whose structure cannot be observed. One may only 
...
the function to (int 0) gives a static value in return). One
solution is to be conservative and always use the |C| compiler to
compile |f| to a future-stage expression, which we can tag with
E1. But this defeats the purpose of partial evaluation as we may miss
...
appears as the argument of |app|). In other words, we have to find a way
to incorporate |f| as it is into the P1 data structure. We would like
to write that when |t| is a particular type |a->b|, this data type
should be 
\begin{code}
data P1 (a->b) = S1 (R (P1 a -> P1 b)) (C (a->b)) 
               | E1 (C (a->b))
\end{code}
That is, we need a non-parametric data type, something akin to
type-indexed functions and type-indexed types, as used in 
\citet{Oliveira2005:TypeCase}, where it is dubbed the \emph{typecase}
design pattern.

The problem we encountered here is quite akin to the typed CPS
...
\citet{Oliveira2005:TypeCase}.  These
...
underlying domain.
The function |buildsimp| checks to see if
...
When doing partial evaluation on |fix|, we made a deliberate decision to `go all
the way' -- so long as the computation can proceed at the PE stage,
