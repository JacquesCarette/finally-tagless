...
\end{comment}
\authorinfo
{\hspace*{-15em}\smash{\normalsize\it\begin{minipage}[t]{30em}
...
\oleg{Emphasize somewhere in the Intro that we solved the problem
exactly as posed in Walid's, Xi's, and SPJ's papers. And we obtain the
result in the form those papers declare desirable.
We use their running example and achieve the result they
call desirable.}
...
\oleg{We must stress that we do not Church-encode a universal type
(or the solution in the previous section). The latter will not be
patently total because we have to provide error continuatiuons and we
have to argue why they will not be called. A compiler is mots likely
incapable of such an argument and so has to compile them in. Here, we do
not have any error continuations at all.}
...
\oleg{We must stress that: of course all parts of our solution are
known. That was the whole point. The problem was claimed to be
unsolvable with the existing means, and so new techniques have been
proposed. We showed that regardless of the merits, power and convenience of
those new techniques, the existing, old solutions were sufficient for
the stated problem -- thus correcting the public record (of ICFP and
POPL conferences). We don't say GADT, etc. are useless -- but we wish
to show to what extent the existing old methods suffice.}
\oleg{Our contribution that we make do with a very restricted polymorphism
(HM plus the module system) which guarantees inference in ML and
Haskell.}
\oleg{So, our thrust is: there is public (ICFP, POPL) record of the problem
being open and claims that solving it requires various
extensions. We show that the problem as formulated in the literature
can be solved with the existing means. We claim the novelty of this,
essentially negative, result. We specifically disclaim any new
inventions.}

Again, the fact that our
...
\oleg{mention TH solution in footnote. ``I was rather hoping it could
have been emulated via TemplateHaskell. I realise TH is untyped and a
compile-time staging system. The claim would have been that the TH
code produces will always be type correct. We could still map the
typed ByteCode data type to untyped TH abstract syntax.''}
Since Haskell
...
\oleg{Rev3:In 4.3, please carefully distinguish the limitation of GHC's
...
The new features may be deservingly
...
Even the idea of having multiple interpretations of the
...
Stress differences from phantom types.
Use of phantom types is popular: see Lava 
Lava: Hardware Design in Haskell (1998)) and Rhiger's thesis.
(Lava: has universal type for Bit and NumSig and correspondent
tag matching: it is not tagless).
...
OR:
The simplicity of our encoding enables trivial proofs of
...
first approach that gives typed CPS in a HM language. We also need to
cite the paper by Stefan Monnier at PLPV2006 where they say, I think,
that typed CPS is impossible without GADT or like.



the basic idea of this paper (the encoding of the term
...
BTW,
many GADT encodings I've seen (Sulzmann's et al) require existentials,
which are not available in ML. The only GADT encodings I've seen in ML
...
syntactic hole.
    Pfenning and Lee conclude that partial evaluation and program
    transformations "do not seem to be expressible" even using their
    extension to F-omega, perhaps because their avoidance of general
    recursive types compels them to include the polymorphic lift that we
    avoid in our Section 4.1.
...
Type-indexed types have been around since at least Harper and
...
intensional type analysis cannot handle HOAS with fix. 
...
embedded interpreters to justify
advanced type systems.  While we admire all this technical work,
we were not convinced that their 
...
Our final approach seems related to \citearound{'s approach to HOAS via
catamorphism and anamorphism}\citet{Washburn-Weirich-boxes}.
...
\citet{Ramsey-ML-module-mania} and \citet{Benton-embedded-interpreters},
partial evaluation by \citet{Danvy-TDPE}, and type-level functions by
\citet{oliveira-typecase}.  As explained in~\S\ref{S:PE-solution}, we
...
foundational work in this important area.  Particularly intriguing is the link to the
...
As \S\ref{S:clarify} observes, higher-rank and higher-kind
...
\citep{shao-typed,shao-transparent}.
...
\oleg{From Rev3}This representation limited to primitive
