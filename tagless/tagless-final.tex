...
In sum, by encoding terms using ordinary functions (the \emph{final}
rather than \emph{initial} approach), we achieve a tagless interpreter
for a typed object language, in a metalanguage with a very simple type
system \citep{hindley-principal,milner-theory}.  All the run-time errors
in \S\ref{tagproblem} (applying a nonfunction; evaluating an open term)
are now reported at compile time. Because this new interpreter uses no
...
functions |b|, |lam|, and so on.  In the remainder of this paper, 
...
MetaOCaml \citep{metaocaml} and Haskell. The complete code is available 
