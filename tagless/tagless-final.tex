...
.<fun x_5 ->
  ...
  (fun x_6 -> (fun x_1 -> x_1 true)
              (fun x_7 -> x_6 x_7 x_5))>.
\end{code}
This output is the na\"{\i}ve CPS transformation of $(\fun{x}x)\True$,
containing several apparent $\beta$-redexes.  To eliminate these
redexes, we simply change~|T| to instantiate |CPST| with |P| instead.
\begin{code}
{P.st = Some <fun>;
...
\item Thus any self-interpreter must have tags and hence cannot be 
  Jones-optimal.
\end{enumerate*}
...
demonstrate the usefulness of HOAS for typed self-interpretation.
However, no claim is made that the source program is itself typed --
just that the interpreter is.  Our reading is that the source language
is not typed. Therefore, object term encoding has tags and the
interpreter (given in the code accompanying
\citet{Danvy-tagging-encoding}) extensively uses partial
pattern-matching. The shift to HOAS, however, lets the partial
evaluator to remove all the tags. In contrast, our object encoding and
evaluators do not have tags to start with and have no possibility of
...
static and dynamic values in parallel.  In some sense,
