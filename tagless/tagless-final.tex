...
\begin{enumerate}
\item to evaluate in direct or continuation-passing styles, 
  compile, and partially evaluate a typed higher-order object language
   in a typed metalanguage with a simple type system. All these
   interpreters have no type tags and never get stuck;
\item a Hindley-Milner--typed partial evaluator with no 
  polymorphic lift operation and with
  the delayed binding-time analysis of functions . The partial
  evaluator needs no advanced type systems such as GADT or intentional
  type analysis;
\item to check statically for well-typed and closed object programs
      using the type system \oleg{typechecker?} of the metalanguage;
...
\item a functor signature that encompasses all our interpreters, from
    evaluation and compilation to partial evaluation and CPS;
...
In contrast to \S\ref{tagproblem}, we encode 
...
accompanying code has the complete code.
...
\oleg{replace the above with the following?}
The comparison of |Symantics| with Figure~\ref{fig:object} clearly
shows that every typed, closed object term can be represented in the
metalanguage. Moreover,
\begin{prop}
If an object term has the object type |t|, its
representation in the metalanguage has the type 
|forall repr. Symantics repr => repr t|
\oleg{or, has the type repr t for some repr. Meta-language embeds all
  object types.}
\end{prop}
...
In general, 
\begin{prop}
  If the definitional interpreter implementing |Symantics| 
  in the soundly typed metalanguage never gets stuck,
...
implementation that |P| is total.  Further, all pattern\hyp matching occurs
