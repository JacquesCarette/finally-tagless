...
\ifshort For simplicity, our main examples will be in MetaOCaml, but
...
which have all appeared in the literature (in particular, \cite{yang-encoding,sumii-hybrid}), to
...
|repr| in the first version of the partial evaluator |P0| as
\texttt{type ('c,'dv) repr 
	= S0 of ('c,'dv) R.repr~}\Verb+|+\texttt{~E0 of ('c,'dv) C.repr}.
...
dynamic term (of type |('c,'a) C.repr|), then we need to convert
...
\texttt{abstr1 : ('c,'dv) repr -> ('c,'dv) C.repr}
...
for a function type |'a->'b| should be either
\vspace*{-0.6em}
\begin{code}
...
can be equivalently implemented either with GADTs or 
type-class functional dependencies
\citep{oliveira-typecase}. The accompanying code shows both
approaches, neither of which is portable to OCaml. In addition,
the problem of nonexhaustive pattern\hyp matching reappears in
in the GADT approach because GHC 6.6.1 cannot see that a particular
