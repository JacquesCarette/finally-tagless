...
unsatisfactory because it does not reflect the type of the encoded term.
...
\else Except for the fundamental definitions in \S\ref{encoding},
our examples will be shown in (Meta)OCaml, 
even though we have implemented each example equivalently in
both (Meta)OCaml and Haskell%
\footnote{an earlier version of this paper switched liberally between 
both languages, as we believed that our claims were made more obvious
by a judicious choice of metalanguage, but the reviewers disagreed,
pointing out that few people are equally conversant in both.}.
We have chosen to use mainly MetaOCaml because it provides
convenient, typed staging facilities.  We will comment on the significant
...
\item We show how the module system of Ocaml and Haskell's type class 
   system can be used to parametrize the semantics of (a particular style of
   syntax encoding of) lamda terms.
\end{enumerate}
...
A referee aptly summarized our paper as ``proposing a way to write a 
...
\oleg{There is now IncopeDB.hs that defines at least R, L and C
 interpreters (no PE at present though) using de Bruijn indices. So we
 can say that (some of) our development is available in de Bruijn
 style. We won't talk about it in the paper though.}
\end{comment}
In contrast to \S\ref{intro}, we use higher-order
...
We find HOAS to be more convenient, though we could have continued to
...
If an object term has object type~$t$, then its
representation in the metalanguage has type 
|forall repr.| |Symantics repr => repr |$t$.
\end{proposition}

\begin{proof}
The rules for deriving a type $t$ from a term $T$ of the object language
correspond \emph{bijectively} to the rules encoded in the signature
|Symantics| (when the latter is read as a set of implications, \`{a} la
Curry-Howard).  In other words, each of the rules from Figure~\ref{fig:object}
corresponds bijectively to one line of |Symantics|.

It is worth noting that, as we use HOAS, rules that normally depend on 
context, in other words rules which are themselves an encoding of a function
from the context to a typing relation, (like those for |lam| and |fix|)
correspond to actual metalanguage functions.
\end{proof}
...
For simplicity, all examples in the rest of this paper will be in
(Meta)OCaml, but the accompanying code shows a full Haskell implementation
as well.
...
we are now ready to present a series of interpreters.  Each interpreter is an
...
\fi 
\enc{comment}
is obvious to the compiler that
...
\fi
\begin{proposition}
If an object term~$e$ encoded (as $e'$) in the metalanguage  has (object)
type~$t$, then evaluating~$e'$ in the interpreter~|R| either continues
...
\fi
\begin{proposition}
  If, for an implementation |I| of |Symantics|, if for all object
  terms $e$ (encoded as $e'$), evaluating $e'$ with |I| always
  terminates, then 
  the type system of the object
  language is sound with respect to the dynamic semantics defined by
  that implementation.
  In other words, the (total) relation induced by |I| is a sub-relation
  of the one expressed by the type system of the object language.
  \jacques{Please check that I have this right}
\end{proposition}
\ifshort\else
These propositions follow immediately from the soundness of the
metalanguage's type system.
...
as the start of this section, with |'sv| taking the place of |repr_pe t|.
The function |abstr|
...
is $\alpha$\hyp equivalent to~$e$ (or in some accounts, no
