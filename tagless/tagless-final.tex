...
an evaluator, a (byte-)compiler and a staged evaluator, a partial
evaluator, a term-size evaluator and a CPS call-by-name and
call-by-value transformers.
...
The particular benefit is ruling out meaningless object terms, and
so enabling faster interpretation. In addition, our interpreters
assuredly do not get stuck.
...
metalanguage, the representation can be performed by hand (e.g., by
entering object terms at a metalanguage interpreter's prompt) or
created by a parser/typechecker reading from a text string.
We leave aside
the issue of whether to program the parser/typechecker using dependent
types \citep{WalidICFP02} or not \citep{baars-typing}.
\oleg{perhaps say more affirmatively: the problem of writing such
  a parser/interpreter has been solved, with dependent types (cite) and
  without (cite), and so will not be discussed here.}
...
\end{code}
We represent each variable using an unary de Bruijn index.
...
self-interpretation (Section~\label{selfinterp}),
