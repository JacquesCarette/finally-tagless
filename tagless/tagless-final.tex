...
intentions and can be checked early by machine (see~\S\ref{encoding}).
...
we achieve a tagless interpreter
...
\ccshan{Need to cite Thiemann here and in related work?}

The interpreter above is wired directly into the
functions |b|, |lam|, |app|, and so on.  In the remainder of this paper, 
we explain how to abstract the interpreter so as
to ``evaluate'' the same term in many different
...
\item and an approach to self-interpretation compatible with the
  above.  Self-interpretation turned out to be harder than expected.
\end{enumerate}
...
The parameter |repr| to the class has the kind |* -> *|.
\begin{code}
...
OCaml as a functor from |Symantics|. This
...
The first interpreter evaluates the object term to its value in the
...
examples, we instantiate the |EX| functor from~\S\ref{encoding} with this |R| module.
\begin{code}
module EXR = EX(R)
\end{code}
...
cannot ever fail and is compiled to no-op.
...
We can also compile our object language
...
Inserting brackets and escapes appropriately into the evaluating
interpreter~|R| above yields the compiling interpreter~|C| below.
...
residual code with no optimization. For example, evaluating our |test1|
\begin{code}
...
code produced for $\fun{x}x$ is simply |fun x_6 -> x_6| and contains no 
  call to the evaluator, unlike the recursive calls to |eval0| and
...
\oleg{where to put the following? Should we even mention this?
If not, we should still keep this in comments.}
...
\subsection{Call-by-name CPS interpreters}
...
possible then to represent a call-by-name (CBN) object language in a
call-by-value (CBV) metalanguage. The answer, given back by Reynolds
and Plotkin (cite) is CPS, which was specifically introduced by them
to make the evaluation strategy of the definitional interpreter
...
same independence in the typed setting \oleg{mention the quote from
  Reynolds that typed setting would be nice?}. Specifically, we
demonstrate a CBN evaluation of our object language in OCaml, which is
CBV. We do so by building a CBN CPS interpreter for the object
language.

The CBN CPS interpretation was introduced by Plotkin (cite). As in the
more familiar CBV CPS, the interpretation of a value is a function
accepting the continuation argument |k| and yielding the answer
...
\end{code}
In the both cases, the returned value has the type 
...
with the Symantics signature, so to fit the CBN CPS interpreter within our
...
|lam f|. We cannot say that the the result of interpreting an object
expression of the type |t| is |(t->'w)->'w| because if |t| happens to
...
function with a type-case distinction. In Section~\ref{S:PE-solution} we have
shown the way to encode such a function, using the additional type
argument |'sv| of the |repr| type. Furthermore, we notice that the type 
function |repr_pe| needed for the partial evaluator,
Section~\ref{S:PE-solution}, is precisely the same type function we
need for CBN CPS. This gives us for the following CBN CPS interpreter,
of the Symantics signature:
...
We could have just as well made |RCN| a functor, parameterized over
the answer type -- as is common when writing CPS in SML. In the
present case, we chose to use higher-rank polymorphism of OCaml record
types.

Because |RCN| has the Semantics signature, we can instnatiate with it and run
all of our previous examples. A more interesting is the following
example.
\begin{code}
...
\end{code}
we never get any result because of looping. In contrast, if we use
the CBN interpreter
\begin{code}
...
CPS transformer. We take any implementation of Symantics (interpreter,
