...
\citep{xi-guarded,WalidPOPL03}. (By the way, if we
...
Let us re-examine the problem in~\S\ref{S:PE-problem}. 
\oleg{If we drop the previous Section, the phrase above should be changed.}
What we
would ideally like is to write
\begin{code}
...
\end{code}
Although we can use type classes to define this type function
in Haskell, that solution is not portable to MetaOCaml. However,
these three typecase alternatives are already present in existing
methods of |Symantics|.
A simple and portable solution thus emerges: we bake |repr_pe| 
into the signature |Symantics|. Switching to MetaOCaml to demonstrate
this portability, we recall from Figure~\ref{fig:ocaml-simple} 
in~\S\ref{encoding} that the |repr| type
constructor took two arguments |'c| and~|'dv|. We add an argument
|'sv| for the result of applying |repr_pe| to~|'dv|.
Figure~\ref{fig:ocaml} shows the new signature.
\begin{figure}
...
it is an evaluator.\oleg{I don't understand this whether
  classification: any interpreter is an evaluator, no? It all depends
  on what the domain is. Anyway, the reviewer suggests rewriting the
  clause as ``whether or not it is an evaluator''. I suggest to drop
  it entirely.}
