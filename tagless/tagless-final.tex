...
\small
    ...
    \rm \ifshort \hfill John Reynolds~\fi\citep{reynolds-definitional}
\end{quote}
...
\begin{itemize*}
    ...
    \item transform the term to continuation\hyp passing style (CPS),
        even call-by-name (CBN) CPS, so to isolate the evaluation
...
\footnote{\texttt{ByteCode} can be mapped to the abstract syntax of Template
...
GADTs. While we admire all this technical work, we were not convinced
