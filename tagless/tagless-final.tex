...
As a side-effect, we provide another convincing application of the 
usefulness of being able to abstract over higher-kinded types.
\end{abstract}
...
\item Show how higher-kinded abstraction can be useful for building
    embedded DSLs.
\end{enumerate}
...
\citet{fiore:nbe-ppdp2002}, nor that of
...
That it is possible to regard our various evaluators as forming a 
family is also an old idea \citep{Holst-AMIX}. \Citeauthor{Holst-AMIX}
introduces \emph{language triplets} as an interesting mechanism to 
relate (untyped) languages in much the same way we do.  Seeds of 
several of the techniques we use here can also be found in this paper.
...
interpretations of them.  \Citet{JonesNielson:94:AbstractInterpretation}
...
In many ways, our work builds on that of \citep{Thiemann-combinators}.
We use the same basic techniques, but our partial evaluator is online
as this is the only way to make partial evaluation ``fit'' in our
family.  In other words, by moving to an online partial evaluator and
using staging instead of a $2$-level language, we are able to show that
\citeauthor{Thiemann-combinators}'s techniques are highly parametric
and typeable using mainstream features.  Using many different type
classes, including constructor classes, \citep{Thiemann-GenClass}
presents a Gofer implementation of set of \emph{binding-time polymorphic}
combinators.  By gathering all their classes into one and dropping the
requirement for a polymorphic |lift|, they could have invented
|Symantics| themselves.
