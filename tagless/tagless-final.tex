...
needs to applied once for each answer type, but it translates
...
thus can be not only evaluated or compiled but also transformed
...
triplets}}\citet{Holst-AMIX}, though in an untyped setting.  In the typed
setting, \citet{Nielson88} uses a typed $\lambda$-calculus as a metalanguage
for expressing families of program analyses; however the embedding of the
object language into the metalanguage, unlike our |Symantics| is not typed.
