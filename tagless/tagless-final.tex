...
It is straightforward to create an algebraic data type, say in OCaml\ifshort\
...
Our solution is \emph{not} Church-encoding the
...
We attack the problem of tagless (staged) typed-preserving
...
\ifshort \BUseVerbatim{P1} \else \UseVerbatim{P1} \fi
...
\ifshort \BUseVerbatim{P} \else \UseVerbatim{P} \fi
...
state. This extended object language adds operations
...
let-form |let x = e1 in e2|. The latter enforces an order of
...
has the form |lapp e1 (fun x -> e2)|, which looks like functional
...
\begin{code}
module EXSI_INT(S: SymSI with type state = int 
                          and type 'c states = int) = 
struct
  open S
...
\end{code}
The accompanying source code shows several more tests, including an
imperative power function that uses state as the accumulator as well as
a test for a higher-order state.

The state-passing interpreter is an extension of the CBN CPS
...
module RCPS (ST: sig 
  ...
  let lapp e2 e1 = {ko = fun k -> e2.ko 
      (fun v -> (app (lam e1) {ko = fun k -> k v}).ko k)}
  let deref () = {ko = fun k s -> k s s}
  let set e = {ko = fun k -> e.ko (fun v s -> k s v)}
end
\end{code}
The implementations of the |int|, |app|, |lam|, etc. functions is
\emph{identical} to those of |RCN| and elided. New here is the extended type
|repr|
which now includes the state, and the ``constructors'' for the imperative
...
evaluated. In the case of first-order state (such as |int|) we 
...
code.  It is worthwhile reiterating how close this implementation is to the CPS
...
\cite{Lava} and by \citet{Rhiger-thesis}. However, these approaches
