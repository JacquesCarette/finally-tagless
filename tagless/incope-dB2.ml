...
assert( 3 = R.runit EXR.test1);;
assert( 4 = R.runit EXR.test2 2);;
assert( 4 = R.runit EXR.test3 succ);;

assert( 8 = R.runit EXR.testgib 1 1 5);;
assert( 8 = R.runit EXR.testgib1);;
assert( 8 = R.runit EXR.testgib2 1 1);;

assert( 128 = R.runit EXR.testpowfix 2 7);;
assert( 128 = R.runit EXR.testpowfix7 2);;
assert( 0   = R.runit EXR.testpowfix0 2);;
...
assert( 3 = L.runit EXL.test1);;
assert( 2 = L.runit EXL.test2);;
assert( 5 = L.runit EXL.test3);;

assert( 17  = L.runit EXL.testgib);;
assert( 23 = L.runit EXL.testgib1);;
assert( 23 = L.runit EXL.testgib2);;

assert( 11  = L.runit EXL.testpowfix);;
assert( 15 = L.runit EXL.testpowfix7);;
assert( 15 = L.runit EXL.testpowfix0);;
...
(* The partial evaluator is parameterized over the module that does
   both the current-stage interpretation ('static') and the 
   future-stage interpertation (so-called `dynamic')
   P(N)(F) produces the instance of Symantics2.
...
(* In addition, a doubly-parametric PE needs to know more about the
   'Now' Symantics, as it needs to be able to re-inject things.
   In other words, the representation for vr is fixed.
*)

module type NowSymantics = sig
  type ('c,'dv) vr= 'dv
  type ('c,'h,'dv) repr = 'h -> ('c,'dv) vr
  val int  : int  -> ('c,'h,int) repr
  val bool : bool -> ('c,'h,bool) repr
...
module P(N:NowSymantics)(F:Symantics) = struct
  type ('c,'sv,'dv) vr = {st: ('c,'sv) N.vr option;
                          dy: ('c,'dv) F.vr}
  type ('c,'h,'sv,'dv) repr = 'h -> ('c,'sv,'dv) vr
  type h0 = H0				(* empty environment *)
  let abstr {dy = x} = x
  let habstr {dy = x} = fun (_:h0) -> x
  let hlift x = fun (_:h0) -> x
  let pdyn x = {st = None; dy = x}
  let close x = x H0
  let vz   = fun (x,_) -> x
  let vs v = fun (_,h) -> v h
  let int  (x:int)  = fun h -> {st = Some (close (N.int x));
                                dy = close (F.int x)}
  let bool (x:bool) = fun h -> {st = Some (close (N.bool x));
                                dy = close (F.bool x)}
  ...
  match (x h,y h) with
  | {st = Some m}, {st = Some n} -> cast (close (f1 (hlift m) (hlift n))) h
  | e1, e2 -> pdyn (close (f2 (habstr e1) (habstr e2)))
  ...
  let add e1 e2 = monoid int 0 (N.add) (F.add) (e1,e2)
  let mul e1 e2 = ring int 0 1 (N.mul) (F.mul) (e1,e2)
  let leq e1 e2 = build bool (N.leq)  (F.leq) (e1,e2)
  let if_ eb et ee = fun h -> match eb h with
            (* It doesn't look like one can use N.if_ ?? *)
