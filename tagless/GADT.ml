...
let rec eval = {ev = 
 function 
   | {ef_int = Some k} -> k (fun eq x -> apply_eq (symm eq) x)
   | {ef_inc = Some k} -> k (fun eq -> apply_eq (symm eq) succ)
   | {ef_lft = Some k} -> k (fun eq x -> apply_eq (symm eq) x)
   | {ef_lam = Some k} -> 
       k {lam_k = 
	  fun (eq,f) -> apply_eq (symm eq) (fun x -> eval.ev (f (e_lft x)))}
   | {ef_app = Some k} -> 
       k {app_k = 
	  fun (eq,f,x) -> apply_eq (symm eq) ((eval.ev f) (eval.ev x))}
	      };;
