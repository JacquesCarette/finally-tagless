...
type ('a,'b) rep = R of ('a,'b) av * labels * cache 
;;

let lab n = function 
  | R (x,(l::ls),c) ->  let l' = string_of_int n in
                  R (x,(l'::ls),([l'],[l])::c);;
...
let analysis_depth = ref 1;;

(* Truncate the list of labels to the given depth
*)
let rec take n = function [] -> []
   | (h::t) -> if n = 0 then [] else h :: take (pred n) t
;;

let truncate lst = take (!analysis_depth + 1) lst;;

let add_ctx ctx_l (l::ls) = l::truncate (ctx_l::ls);; 
...
let app e1 e2 =
 ...
 | (R (D f,lf,cachef), (R (xs,lx,cachex) as arg)) ->
     let newlabel = gensym () in
     let R (rs,lr,cacher) = f (R (xs,(add_ctx newlabel lx),cachex)) in
     R (rs,[newlabel],([newlabel],(add_ctx newlabel lr))::
	(List.map (fun (l1,l2) -> (add_ctx newlabel l1,l2)) cacher) @ cachef)
;;
