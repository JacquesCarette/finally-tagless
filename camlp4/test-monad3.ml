...
(* OO version no longer supported 
let test_state3 () =
...
(* repeat some of the above tests with named monads *)
module StateMonad = struct
  let bind ma fmb = fun state -> let result, newstate = ma state in 
                                 fmb result newstate
end
...
let all_tests () =
  ...
  testM_two_element_list () = [-2; -3; -4; -1; -2; -3; 0; -1; -2] &&
  simple_mdo () = 2 &&
  simple_let_mdo () = 2 &&
