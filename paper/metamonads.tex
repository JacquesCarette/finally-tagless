...
\vspace*{-2pt}
\begin{itemize}
    \item Extending a let-insertion monad of
      \cite{MSP:PADL04,KiselyovTaha} for generating control structures
      such as loops and conditionals. The extension is non-trivial: e.g.,\\
...
The monad in this paper is similar to the one used in
\cite{MSP:PADL04,KiselyovTaha}.  However, the latter works used only
|retN| of all monadic operations, and fixpoints (for performing
iterations at generation time).  This paper does not involve monadic
fixpoints (because the generator is not recursive) but makes extensive
use of monadic operations for generating conditional and looping
operations.
