...
The |do|-notation of Haskell (cire report?) shows that it is possible
...
In the present paper, we use the monad that is the CPS monad with
another layer of state. We will discuss the need for the state in
Section XXX. The type of the monad is:
\begin{code}
type ('v,'s,'w) monad = 's -> ('s -> 'v -> 'w) -> 'w
\end{code}

The monad is just the function of two arguments: the state (of type
|'s|) and the continuation. The continuation receives the current
state, the value (of type |v|) and yields the answer of the type |w|.
The monad is polymorphic over all these three type parameters. Except
a few primitime monad operations below, all other code treat the monad
as an abstract data type.


Monads represent computations that can have ``effects''. In our case,
we can have ``state effects'' (update of state) and ``control effect''
(which occurs whenever the computation does something else but
invoking its continuation once). As we will see, ``control effects''
in our case manifest themselves as ``code generation effects''. 
...
The two monad operations, |ret| and |bind| are conventional: |ret a|
returns a computation that takes the current state and the
continuation and immediately invokes the continuation passing it the
unmodified state and the value. Obviously, the computation |ret a| has
no control effects. The operation |bind a f| combines a
monad |a| of the type |(s,v,w) monad| and the function |f| of the
type |v -> (s,v1,w) monad| into a bigger |(s,v1,w) monad|. The latter
will execute computation |a|, pass it result to |f| and execute the
latter's computation. The state facet of our monad is handled with two
operations: 
...
|fetch| is the computation that yields the current state and |store v|
is the computation that updates the current state and yields |()|. We
model the state as a list (which is actually a list of polymorphic
variants, as we shall see later).
...
Finally, we should be able to run our monad -- that is, perform the
computation of teh monad and get the result -- which in our case is
the code expression. We run the monad by passing it initial state (the
empty list) and the initial continuation |let k0 s v = v|.

We can now re-write our |param_code2| example of the previous section
as
...
let param_code3 plus one =
  ...
  and run m = m [] k0 
  in .<fun x y -> .~(run (gen .<x>. .<y>.))>.
\end{code}

That does not seem like the improvement. But here we can introduce the
do notation, which is patterned after the do-namtion of Haskell.
\oleg{Explain our mdo briefly? Refer to the message on the
  Ocaml-list?. Refer to camlp4 and note the implementation}
