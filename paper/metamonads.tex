...
the matrix, whether to compute and return the detrminant or rank, how
...
The survey \cite{carette04} of
Gaussian elimination implementation in an industrial package Maple
found XXX various aspects and XXX different implementations of the
algorithm.  We can manually write each of these implementations
optimizing for particular aspects and using cut-and-paste to ``share''
similar pieces of code. \oleg{I don't think we need to say much here
-- the shear number of various implementations should be enough of a
clue.} We can write a very generic GE procedure that accounts for
...
high-performance numerical computing (cite? At least carette04). 

A more appealing (acceptable? indispensable? need a better word)
approach is generative programming
...
preprocessor (cite XXX), C++ template meta-programming, Template
...
it worthwhile to use full pivoting on a matrix of rational integers
($\mathbb Q $).
...
Appendices give samples of the generated code (which is availablke in
full at XXX).
...
Now that we have the appropriate basic techniques for
writing our code generators, we need to worry about modularization.
Here we can really take advantage of the fact that
MetaOCaml is a staged language: it is not particularly relevant how much
...
\vspace*{-6pt}
	\item \textbf{Domain}: In which (algebraic) domain do the matrix
		...
		surveyed 20 different domains were encountered.
		% \oleg{citation?} \jacques no citation needed, carette04 is
		% already cited above.
...
and potentially a relative size measure.  We need at least
\emph{exact} division to exist, but if full division is available, that
can be used as well.  Normalization is frequently trivial for domains
where elements have unique representations, but for some
domains (like polynomials in sum-of-product form) this
is not the case.  The simplest case of such an abstraction was given
in Fig.~\ref{easycode}, |param_code1|. In the latter, code-generators such as
...
\emph{structures} (i.e., modules) 
% (cite Ocaml module system tutorial?)
% normally yes, but our references are what's making this long right now!
so we can take advantage of extensions, type abstraction and constraints,
and especially parameterized structures (\emph{functors}).
We first define the type of the domain, the signature |DOMAIN| which
...
|DOMAIN| signature) is the |int| domain. The notation
...
missing ``methods'' or methods with incorrect types will be caught
...
|float| and |Num.num| (arbitrary precision exact rational numbers)
are equally simple, and with scarcely
more work, $\mathbb{Z}_p$, $\mathbb{Z}\left[x\right]$,
$\mathbb{Q}\left(x\right)$ as well more complex domains can be implemented.
...
example, a piece of code for the value of the type |Dom.v ref| among
...
\end{code}
\vspace*{-4pt}
This structure implements an update policy that can use the 
|Dom.div| operation without
restrictions -- which is possible only if the domain has such an
operation. A domain such as the integer domain may still provide |Dom.div|
of the same type, but that operation can only be used when we are sure
that the division is exact. But using it with
|DivisionUpdate| will lead to a compile-time error, when compiling
...
``compiled'' as |!. GenIA1.gen| or with off-shoring. The code
% (cite).  No space!
...
and used fixpoints (for performing iterations at generation time).
...
knowledge can also be encoded.  However inlining critically depends on
the compiler's fully inlining of all methods, and as has been reported
in the literature, this can be challenging to insure. Furthermore, all errors
...
to the ones in the generator itself. Furthermore, as templates
cannot be local, a lot of code generation is scattered around instead
...
%   Mention that nobody before used functors to abstract
