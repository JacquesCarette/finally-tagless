...
computing the determinant and the rank of the matrix. All members of
the family share the same pattern of applying elementary row
operations to all rows of the matrix in order. The individual
algorithms differ in the output, in application of pivoting, in
algebraic domain and the use of full division. Modern architectures
demand further splits \oleg{specializations? instantiations?
  divisions?} of the family for particular matrix layouts, e.g.,
sparsed or tiled.
...
domain-specific knowledge and so cannot reliably \oleg{assuredly?}
be done by the compiler \oleg{cite telescopic languages, active
  libraries, or at least scp-CohenDGHKP06?}.

\begin{comment}
% Old text
Furthermore, already
in linear algebra, there is a wealth of different aspects 
such as pivoting, algebraic domain, and matrix representation
that \emph{may}\oleg{indeed, why may is emphasized?} need to be
addressed. Implementations of the
...
\begin{itemize}
    ...
    \item Encode domain-specific knowledge in the generators so to
      catch domain-specific instantiation errors at generation
      time.
    \item Provide a thorough calssification of the family of LU decomposition
      algorithms.
\end{itemize}
