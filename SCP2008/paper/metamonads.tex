...
on programmings with aspects and sum up our guiding methodology.
...
|IntegerDomain| is indeed a |DOMAIN|. 
The constraint |DOMAIN| may be omitted; in that case, the
...
lists~\cite{mlton-proplist}.  A property list, too, represents an
...
literally apply to us however. MetaOCaml's environment classifiers
...
We have two instances of |Det|. The first one is used to specify
...
module AbstractDet =
  ...
  let decl () = perform
      ddecl <-- retN (liftRef oneL); (* track the sign: +1, 0, -1 *)
...
both instances of |Det| contain |fin ()| function to generate
...
since all linear algebra codes are parameterized by the
...
or with off-shoring. The code for |resIV5|
...
\cite{MSP:PADL04,KiselyovTaha}.  However those papers used only
...
\bibitem{Veldhuizen:1998:ISCOPE}
...
\bibitem{musser94algorithmoriented}
...
\bibitem{conf/dagstuhl/CzarneckiOST03}
...
C.~Calcagno, W.~Taha, L.~Huang, X.~Leroy, Implementing multi-stage languages
  using asts, gensym, and reflection, in: K.~Czarnecki, F.~Pfenning,
  Y.~Smaragdakis (Eds.), Generative Programming and Component Engineering
  (GPCE), LNCS, Springer-Verlag, 2003.

\bibitem{metaocaml-org}
{MetaOCaml}, \url{http://www.metaocaml.org}.

\bibitem{TahaSheard97}
W.~Taha, T.~Sheard, Multi-stage programming with explicit annotations, in:
  ...
  Manipulation {(PEPM)}, ACM Press, Amsterdam, 1997, pp. 203--217.
...
\bibitem{Veldhuizen:2004}
T.~L. Veldhuizen, Active libraries and universal languages, Ph.D. thesis,
  Indiana University Computer Science (May 2004).
\newline\urlprefix\url{\url{http://osl.iu.edu/~tveldhui/papers/2004/dissertati%
on.pdf}}

\bibitem{KiselyovTaha}
O.~Kiselyov, K.~N. Swadi, W.~Taha, A methodology for generating verified
  ...
  international conference on Embedded software, ACM Press, New York, NY, USA,
  2004, pp. 249--258.

\bibitem{Padua:MetaOcaml:04}
A.~Cohen, S.~Donadio, M.~J. Garzara'n, C.~Herrmann, D.~Padua, In search for a
  program generator to implement generic transformations for high-performance
  computing, metaOCaml Workshop, October 2004.

\bibitem{CaretteKiselyov05}
J.~Carette, O.~Kiselyov, Multi-stage programming with {F}unctors and {M}onads:
  eliminating abstraction overhead from generic code, in: Generative
  Programming and Component-based Engineering {GPCE}, 2005, pp. 256--274.

\bibitem{MSP:PADL04}
J.~L. Eckhardt, R.~Kaiabachev, K.~N. Swadi, W.~Taha, O.~Kiselyov, Practical
  aspects of multi-stage programming, rice University Techical Report
  {TR05}-451,
  \url{http://www.cs.rice.edu/~taha/publications/preprints/2004-02-16.pdf}
  (Feb. 2004).

\bibitem{metamonadsURL}
...
G.~Kiczales, J.~Lamping, A.~Menhdhekar, C.~Maeda, C.~Lopes, J.-M. Loingtier,
  ...
  Springer-Verlag, Berlin, Heidelberg, and New York, 1997, pp. 220--242.
...
E.~W. Dijkstra, On the role of scientific thought, published as
  \cite{EWD:EWD447pub}.
\newline\urlprefix\url{\url{http://www.cs.utexas.edu/users/EWD/ewd04xx/EWD447.%
PDF}}
...
R.~Gl{\"u}ck, R.~Nakashige, R.~Z{\"o}chling, Binding-time analysis applied to
  mathematical algorithms, in: System Modelling and Optimization, 1995.
