...
section~\ref{related}. We then outline future work. In our
conclusion (section~\ref{conclusion}), we sum up our work, comment
on programmings with aspects, and outline the overall methodology
we used to structure our work.
...
semantic error when a user specifies that the determinant should be computed
and returned and yet specifies the |NoDet| aspect which tracks no
determinant at all.

It is also worth noticing the |INTERNAL_FEATURES| type, which bundles
various information tracking aspects.  These are in some ways rather
intruiging: they are purely internal to the algorithms, yet they are the
ones which
...
aspects.  While writing the code for these was straightforward enough,
this required quite a bit of type-level infrastructure, as well as the
preflight checks%
\footnote{We had originally~\cite{CaretteKiselyov05} implemented all our
preflight tests at the type level, but this did not scale.}
...
representation) are the most obvious, it is quite
...
efficiency.  Interestingly, in conversations that the first author had
with D. Parnas, we discovered this is apparently
what~\cite{journals/cacm/parnas72a} was advocating.  We believe that we are
taking the first steps towards a \emph{typed yet efficient} realization of
these ideas, where the various design-time entities can be directly encoded in
