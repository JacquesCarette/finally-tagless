...
\oleg{Perhaps we should just add the following phrase, paraphrased
  from your remark:}
...
\oleg{I'd rather keep the following lines in the comments. I know
  Jacques has been credited, but I don't have the exact reference
  where he proposed it.}
%% (cited from
...
expression |assert (is_prime P.p)|, to be called an initializing
...
The module of the signature |INTERNAL_FEATURES| bundles information
tracking aspects. The latter are not directly selectible by the
user. Rather, they are \emph{function} of other user choices. The
...
not a field). Currently we use pre-flight checks to ensure consistency
...
order is irrelevant, but all aspects suc as |Det| must be
