...
this time with HOAS rather than deBruijn indices. The language is close 
...
The previous test now reads |app (lam (\x -> x)) (bool True)| and the
inferred type is |Symantics repr => repr Bool|.
...
OCaml: functors (give the signature of
...
\end{code}
The unit in |testpowfix| is for the sake of value restrcition (which
interferes with the environment classifier inference).

Thus the DSL expression has the type of the functor, from |Symantics|
to the value.
