...
module FloatDomainL = struct
    ...
    let better_thanL = Some (fun x y -> (fun () -> abs_float (x ()) < abs_float (y ()) ) )
...
module IntegerDomainL = struct
    ...
    let uminusL x = (fun () -> - x ())
    let divL x y = (fun () -> x () / y ()) 
    let better_thanL = Some (fun x y -> (fun () -> abs (x ()) > abs (y ()) ) )
    let normalizerL = None 
end
...
module type CONTAINER2D = sig
  ...
  val row_iter : 'a vc -> ('a, int) rep -> 
      ('a, int) rep -> ('a, int) rep ->
      (('a,int) rep -> 'a vo -> 's -> ('s -> 'w -> 'w) -> ('a, 'e) rep) ->
          (('a,unit) rep, 's, 'k) monad
  val col_head_set : 'a vc -> ('a,int) rep -> ('a,int) rep -> 'a vo -> 
            ('a,unit) rep
  val col_iter : 'a vc -> ('a, int) rep -> 
      ('a, int) rep -> ('a, int) rep ->
      (('a,int) rep -> 'a vo -> 's -> ('s -> 'w -> 'w) -> ('a, 'e) rep) ->
          (('a,unit) rep, 's, 'k) monad
end
...
module GenericVectorContainer(Dom:DOMAINL) =
  ...
  (* only set the head of the current column *)
  let col_head_set x n m y = fun () -> ((x ()).arr).((n ())* (x ()).m + m ()) <-
      y ()
  let col_iter b j low high body = 
    let newbody k = perform
        bjk <-- ret (getL b j k);
