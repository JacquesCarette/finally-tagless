(* Same as funct4 except that there are no .< >. at all *)
...
module type DOMAIN = sig
  ...
  val better_than : ('a vc -> 'a vc -> (bool, 's, 'w) monad) option
  val normalizerf : (v -> v) option
...
module type CONTAINER2D = functor(Dom:DOMAIN) -> sig
  ...
  type 'a vc = contr
  type 'a vo = Dom.v
  val get : 'a vc -> int -> int -> ('a vo,'s,'w) monad
  val get' : 'a vc -> int -> int -> 'a vo
  val set : 'a vc -> int -> int -> 'a vo -> (unit, 's, 'w) monad
  val set' : 'a vc -> int -> int -> 'a vo -> unit
  val dim1 : 'a vc -> int
  val dim2 : 'a vc -> int
  val mapper : (Dom.v->Dom.v) option -> 'a vc -> 'a vc
  val copy : 'a vc -> 'a vc
  val swap_rows_stmt : 'a vc -> int -> int -> unit
  val swap_cols_stmt : 'a vc -> int -> int -> unit
end
...
module type DETERMINANT = sig
  ...
  type 'a lstate
  val decl : unit -> 
    (unit, [> `TDet of 'a lstate ] list, 'b) monad
  val upd_sign : unit -> 
    (unit, [> `TDet of 'a lstate ] list, 'b) monad
  val zero_sign : unit -> 
    (unit, [> `TDet of 'a lstate ] list, 'b) monad
  val acc : indet -> 
    (unit, [> `TDet of 'a lstate ] list, 'b) monad
  val get : unit ->
    (tdet, [> `TDet of 'a lstate ] list, 'b) monad
  val set : indet -> 
    (unit, [> `TDet of 'a lstate ] list, 'b) monad
  val fin : unit -> 
    (outdet, [> `TDet of 'a lstate ] list, 'b) monad
end

(* no need to make the type abstract here - just leads to other problems *)
module type RANK = sig
  type 'a lstate = int ref
  val rfetch : unit -> 
    (int ref,[> `TRan of 'a lstate ] list, 'w) monad
  val decl : unit -> 
    (int ref,[> `TRan of 'a lstate ] list, 'w) monad
  val succ : unit -> 
    (unit,[> `TRan of 'a lstate ] list, 'w) monad
  val fin : unit ->  
    (int,[> `TRan of 'a lstate ] list, 'w) monad
end
...
module type UPDATE = sig
    ...
    val update : ctr -> 'a idx -> 'a idx -> 'a idx -> 'a idx ->
        (unit, [> `TDet of 'a D.lstate] list, 'b) monad
    val update_det : baseobj -> 
        (unit, [> `TDet of 'a D.lstate] list, 'b) monad
end
...
module type TRACKPIVOT = sig
  type 'a lstate
  val decl : unit -> (unit,[> `TPivot of 'a lstate] list, 'w) monad
  val add : perm -> 
    (unit,[> `TPivot of 'a lstate] list, 'w) monad
  val fin : unit -> 
    (perm list,[> `TPivot of 'a lstate] list,'w) monad
end

module TrackPivot = 
  struct
  type 'a lstate = perm list ref
        (* the purpose of this function is to make the union open.
           Alas, Camlp4 does not understand the :> coercion notation *)
 ...
 *)
 val findpivot : 'a C(Dom).vc -> int -> int -> int -> int ->
   (Dom.v option ,[> `TDet of 'a D.lstate] list,'w) monad
end
