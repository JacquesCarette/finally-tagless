...
.<fun a_1 ->
   ...
   t_5>.
# - : ('a, Funct4.ArrayContainer.contr -> Funct4.DetOutput.res) code =
.<fun a_1 ->
   let t_2 = (ref 0) in
   let t_3 = (ref 0) in
   let t_5 = (Array.map (fun x_4 -> (Array.copy x_4)) (Array.copy a_1)) in
   ...
   while (((! t_2) < t_6) && ((! t_2) < t_7)) do
    let t_9 = (ref (-1)) in
    begin
     for j_10 = (! t_2) to (t_7 - 1) do
      if (not ((t_5.(j_10)).(! t_3) = 0.)) then
       if (((! t_9) == (-1)) ||
            ((abs_float (t_5.(j_10)).(! t_3)) <
              (abs_float (t_5.(! t_9)).(! t_3)))) then
        (t_9 := j_10)
       else ()
      else ()
     done;
